...
import AST.NodoIf;
import AST.NodoLlamada;
import AST.NodoPrimLiteral;
import AST.NodoPrimLlamada1;
import AST.NodoPrimLlamada2;
import AST.NodoPrimLlamada3;
...
public class SyntacticalAnalyzer {
        ...
        private NodoExpPrimario primario() throws LexicalError, SyntaxError {
    		...
    		case "(":
    			...
    			match(")");
    			llamadas = llamadaL();
    			return new NodoPrimLlamada1(expr, llamadas);
    		case "new":
    			match("new");
    			kid = match("id");
    			args = argsActuales();
		...
		SyntaxError {
        	...
        	case "(":
        		args = argsActuales();
        		llamadas = llamadaL();
        		return new NodoPrimLlamada3(k, args, llamadas);
        	default:
        		llamadas = llamadaL();
        		return new NodoPrimLlamada2(k, llamadas);
        	}

        }
        
        private List<NodoLlamada> llamadaL() throws LexicalError, SyntaxError {
    		switch (lookAhead.getName()) {
    		case ".":
    			NodoLlamada nodoLlamada = llamada();
    			List<NodoLlamada> llamadas = llamadaL();
    			llamadas.add(0,nodoLlamada);
    			return llamadas;
    		default:
    			return new LinkedList<NodoLlamada>();
    		}
    	}
        
        private NodoLlamada llamada() throws LexicalError, SyntaxError {
                match(".");
                Token kid = match("id");
                List<NodoExpresion> args =argsOpcionales();
                return new NodoLlamada(kid, args);
        }
